...
*/
	...
	TH2D* dE_E_tempS = new TH2D("dE_E_tempS", "dE_E_tempS", 200, 0, 20,250,0,15);
	TH2D* dE_E_tempU = new TH2D("dE_E_tempU", "dE_E_tempU", 200, 0, 20,250,0,15);
	TH2D* dE_E_tempD = new TH2D("dE_E_tempD", "dE_E_tempD", 200, 0, 20,250,0,15);

	TH2D* b_test = new TH2D("b_test", "b_test", 300, 0, 20,300,0,20);
	...
	TH2D* kinematics = new TH2D("kinematics", "kinematics", 90, 90, 180, 200, 0, 20);
	TH1D* Q_value = new TH1D("Q_value", "Q_value", 200, -6, 6);
	TH1D* Q_value_singles = new TH1D("Q_value_singles", "Q_value_singles", 200, -6, 6);
				...
				for(int b =0;b<E_strip_UpB.size();b++)
				{
					for(int c =0;c<E_cal_back_strip_UpB.size();c++)
					{
						...
						if(((dE_strip_UpB.at(a) == 2*(5-E_strip_UpB.at(b)) || dE_strip_UpB.at(a) == 2*(5-E_strip_UpB.at(b))-1) && cal_energy_BB10.at(a)>=0.4) && SX3_prots->IsInside(E_cal_back_en_UpB.at(c),cal_energy_BB10.at(a)))
						{
							...
							if((trig&2)==2) kinematics->Fill(angle_UpB,E_cal_back_en_UpB.at(c)+cal_energy_BB10.at(a));
							if((trig&2)==2) dE_E_tempU->Fill(E_cal_back_en_UpB.at(c),cal_energy_BB10.at(a));
							...
							if((trig&2)==2) Q_value->Fill(qValue);
				...
				for(int b =0;b<E_strip_DownB.size();b++)
				{
					for(int c =0;c<E_cal_back_strip_DownB.size();c++)
					{
						...
						if(SX3_prots_down->IsInside(E_cal_back_en_DownB.at(c),cal_energy_X3.at(a)))
						{
							...
							//cout << angle_DownB << endl;

							if((trig&2)==2) kinematics->Fill(angle_DownB,E_cal_back_en_DownB.at(c)+cal_energy_X3.at(a));
							if((trig&2)==2) b_test->Fill(E_en_DownB.at(b), E_cal_back_en_DownB.at(c));
							if((trig&2)==2) dE_E_tempD->Fill(E_cal_back_en_DownB.at(c),cal_energy_X3.at(a));
			...
			for(int k=0;k<dE_strip_SIDAR.size();k++)
			{
				for(int l=0;l<E_strip_SIDAR.size();l++)
				{
					if(fabs(dE_strip_SIDAR.at(k)-E_strip_SIDAR.at(l))<=1 && cal_dE_energy_SIDAR.at(k) >= 0.4 && cal_E_energy_SIDAR.at(l) >= 0.8 && SIDAR_prots->IsInside(cal_E_energy_SIDAR.at(l),cal_dE_energy_SIDAR.at(k)))
					{
						...
						if((trig&2)==2) kinematics->Fill(angle_SIDAR,cal_dE_energy_SIDAR.at(k)+cal_E_energy_SIDAR.at(l));
						dE_E_tempS->Fill(cal_E_energy_SIDAR.at(l),cal_dE_energy_SIDAR.at(k));
