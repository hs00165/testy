...
*/
	...
	TH2D* q_Value = new TH2D("q_Value", "q_Value", 180, 0, 180, 300, -10, 10);
	...
	TH2D* ds_dbE = new TH2D("ds_dbE", "ds_dbE", 49, 0, 49, 500, 0, 20);

	TH2D* ds_dbP = new TH2D("ds_dbP", "ds_dbP", 49, 0, 49, 500, -2, 2);
	TH2D* ds_dbP2 = new TH2D("ds_dbP2", "ds_dbP2", 49, 0, 49, 500, -2, 2);
	...
	for(int d=1;d<13;d++)
	{
		for(int s=1;s<5;s++)
		{
			sprintf(hname,"D%d_%d",d,s);
			Pos_spec[d][s] = new TH1D(hname,hname,250,-2,2);
		}
	}
   ...
   SIDAR_prots->SetPoint(12,0.723695,2.18029);
	...
	for(unsigned long long int i=0;i<nEntries;i++)
	{
		...
		for( int l=0;l<BarrelDown_vect_size;l++)
		{
			...
			{
				for(int c =0;c<E_back_strip_DownB.size();c++)
				{

					Pos_spec[detID_DownB][E_strip_DownB.at(b)]->Fill(E_pos_DownB.at(b));


					ds_dbP->Fill(E_strip_DownB.at(b) + ((detID_DownB-1)*4), E_pos_DownB.at(b));

					ds_dbE->Fill(E_strip_DownB.at(b) + ((detID_DownB-1)*4), E_cal_back_en_DownB.at(c));


					if(E_cal_back_en_DownB.size() == 0) ds_dbP2->Fill(E_strip_DownB.at(b) + ((detID_DownB-1)*4), E_pos_DownB.at(b));
				}
				...
				for(int b =0;b<E_strip_DownB.size();b++)
				{
					for(int c =0;c<E_cal_back_strip_DownB.size();c++)
					{
						...
						if(SX3_prots_down->IsInside(E_cal_back_en_DownB.at(c),cal_energy_X3.at(a)))
						{
							...
							qValue = (1.012)*(E_cal_back_en_DownB.at(c)+cal_energy_X3.at(a)) - 45.36 - 13.316*sqrt(E_cal_back_en_DownB.at(c)+cal_energy_X3.at(a))*cos(angle_DownB*(3.1415/180.0));
							...
							{
								kinematics1->Fill(angle_DownB,E_cal_back_en_DownB.at(c)+cal_energy_X3.at(a));
								OR_flag = 1;
	...
	TFile write("Downstream_position_plots1.root", "recreate");

	int Nbins, bin_cont, prev_bin_cont, max_bin, max_bin_cont;
	int left_shoulder, right_shoulder;
	double deriv, max_deriv, min_deriv;
	...
	for(int d=1;d<13;d++)
	{
		for(int s=1;s<5;s++)
		{
			Nbins = Pos_spec[d][s]->GetXaxis()->GetNbins();
			window_width = 10;
			max_bin = Pos_spec[d][s]->GetMaximumBin();
			max_bin_cont = Pos_spec[d][s]->GetBinContent(max_bin);
			...
			{
				bin_cont = Pos_spec[d][s]->GetBinContent(bin);
				prev_bin_cont = Pos_spec[d][s]->GetBinContent(bin - 1);

				//Running a derivaive filter over the spectrum, to approximately locate the edges
				deriv = (((Pos_spec[d][s]->GetBinContent(bin+1)) + (Pos_spec[d][s]->GetBinContent(bin+2)))/2.0) - (((Pos_spec[d][s]->GetBinContent(bin-1)) + (Pos_spec[d][s]->GetBinContent(bin-2)))/2.0);
			...
			sprintf(Lfname,"Left_D%d_%d",d,s);
			sprintf(Rfname,"Right_D%d_%d",d,s);

			TF1 *funcL = new TF1(Lfname, flatTopGaussLeft, (Pos_spec[d][s]->GetBinCenter(left_shoulder - window_width)), (Pos_spec[d][s]->GetBinCenter(left_shoulder + window_width)), 3);
			TF1 *funcR = new TF1(Rfname, flatTopGaussRight, (Pos_spec[d][s]->GetBinCenter(right_shoulder - window_width)), (Pos_spec[d][s]->GetBinCenter(right_shoulder + window_width)), 3);

			funcR->SetParameters(Pos_spec[d][s]->GetBinContent(right_shoulder),Pos_spec[d][s]->GetBinCenter(right_shoulder),0.016);
			funcL->SetParameters(Pos_spec[d][s]->GetBinContent(left_shoulder),Pos_spec[d][s]->GetBinCenter(left_shoulder),0.016);

			//funcR->SetParameter(0, Pos_spec[d][s]->GetBinContent(right_shoulder));
			//funcR->SetParameter(1, Pos_spec[d][s]->GetBinCenter(right_shoulder));
			//funcR->SetParameter(2, 2 * Pos_spec[d][s]->GetBinWidth(right_shoulder));

			//funcL->SetParameter(0, Pos_spec[d][s]->GetBinContent(left_shoulder));
			//funcL->SetParameter(1, Pos_spec[d][s]->GetBinCenter(left_shoulder));
			//funcL->SetParameter(2, 2 * Pos_spec[d][s]->GetBinWidth(left_shoulder));

			Pos_spec[d][s]->Fit(Rfname, "qr");
			Pos_spec[d][s]->Fit(Lfname, "qr+");

			Pos_spec[d][s]->Write();

			right_edge = funcR->GetX(0.7*funcR->GetParameter(0),funcR->GetParameter(1),funcR->GetParameter(1) + 5 * funcR->GetParameter(2));
			left_edge = funcL->GetX(0.7*funcL->GetParameter(0),funcL->GetParameter(1) - 5 * funcL->GetParameter(2), funcL->GetParameter(1));

			cout << "D"<< d << "S" << s << "  Left edge:  " << left_edge << "   " << right_edge << endl; 
