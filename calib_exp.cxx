...
*/
   ...
   SIDAR_prots->SetPoint(12,0.723695,2.18029);
	...
	for(unsigned long long int i=0;i<nEntries;i++)
	{
		...
		//if((trig&2)==2) cout << UpMCP << " " << DownMCP <<" "<< XF_scint <<" "<< RF << endl; 
				...
				for(int b =0;b<E_strip_DownB.size();b++)
				{
					...
					//{
						...
						if(SX3_prots_down->IsInside(E_en_DownB.at(b),cal_energy_X3.at(a)))
						{
							...
							qValue = (1.012)*(E_en_DownB.at(b)+cal_energy_X3.at(a)) - 45.36 - 13.316*sqrt(E_en_DownB.at(b)+cal_energy_X3.at(a))*cos(angle_DownB*(3.1415/180.0));
							...
							{
								kinematics1->Fill(angle_DownB,E_en_DownB.at(b)+cal_energy_X3.at(a));
								OR_flag = 1;
	...
	{
		cout << "cannot open expected kinematics text file, you doughnut..." << endl;
		return 1;
	}
