...
void calib_auto_step()
{
	...
	cout << endl;
	cout << "***************************************************************************" << endl;
	cout << "                           84Se SX3 Auto Calibration                       " << endl;
	cout << endl;
	...
	for(int d=1;d<13;d++)
	{
		for(int s=1;s<5;s++)
		{
			sprintf(hname,"D%dS%d",d,s);
			E_spec[d][s] = new TH1D(hname,hname,1000,1000,3000);
		}
	}
...
*/
	...
	ifstream SX3_calib_params2 ("GM_UU_res_strip_cal_params_f.txt");

	if(!SX3_calib_params2)
	{
		cout << "Cannot open Super X3 calibration file, you doughnut..." << endl;
		return 1;
	}
	...
	//=====================READING IN THE CALIBRATION FILE=========================
	while(getline(SX3_calib_params2,line))
	{
		...
		{	
			...
			{			
				...
				offX_new = offX - (new_SX3_cal_params[C_det][strip_no][1] / (2 * new_SX3_cal_params[C_det][strip_no][0]));
				offY_new = offY - (new_SX3_cal_params[C_det][strip_no][1] / (2 * new_SX3_cal_params[C_det][strip_no][0]));

				outfile <<"Res. Strip #"<< strip_no <<"   "<< offX_new <<"   "<< offY_new <<"   "<< slopeGM <<"   "<< new_SX3_cal_params[C_det][strip_no][0] <<"   "<< LERP <<"   "<< RERP << endl;;

				//cout <<"Res. Strip #"<< strip_no <<"   "<< offX <<"   "<< offY <<"   "<< slopeGM <<"   "<< new_SX3_cal_params[C_det][strip_no][0] <<"   "<< LERP <<"   "<< RERP << endl;;
