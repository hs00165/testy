...
void calib_auto_step_BSX3()
{
	...
	ifstream back_SX3_calib_params ("SX3_back_fineEcal.txt");
	...
	Chain1->Add("/media/harry/GODDESS_ANA1/84Se/cal1/alpha_cal_E_1.root"); 
	//Chain1->Add("/media/harry/GODDESS_ANA1/84Se/cal1/alpha_cal_dE_1.root"); 	

	TH2D* E_vs_strip = new TH2D("E_vs_strip", "E_vs_strip", 49, 0, 49,500,0,10);
	TH1D* E_test = new TH1D("E_test", "E_test" ,300,0,8);
	...
	for(int d=1;d<13;d++)
	{
		for(int s=1;s<5;s++)
		{
			for(int t=1;t<5;t++)
			{
				sprintf(hname,"E_D%dS%dF%d",d,s,t);
				E_spec[d][s][t] = new TH1D(hname,hname,1000,500,1500);
			}
		}
	}
	...
	for(unsigned long long int i=0;i<nEntries;i++)
	{
		...
		for( int l=0;l<BarrelUp_vect_size;l++)
		{
			...
			for(int g=0;g<E_back_strip_UpB.size();g++)
			{
				for(int f=0;f<E_strip_UpB.size();f++)
				{
					...
					//{
						...
						//E_spec[detID_UpB][E_back_strip_UpB.at(g)][E_strip_UpB.at(f)]->Fill(E_back_en_UpB.at(g));
			...
			{
				E_test->Fill(E_cal_back_en_UpB.at(c));
			}
		...
		for( int l=0;l<BarrelDown_vect_size;l++)
		{
			...
			for(int g=0;g<E_back_strip_DownB.size();g++)
			{
				for(int f=0;f<E_strip_DownB.size();f++)
				{
						...
						E_spec[detID_DownB][E_back_strip_DownB.at(g)][E_strip_DownB.at(f)]->Fill(E_back_en_DownB.at(g));
				}
			}

			for(int c =0;c<E_cal_back_strip_DownB.size();c++)
			{
				E_test->Fill(E_cal_back_en_DownB.at(c));
			}
	...
	double new_SX3_cal_params[14][5][5][2];
	...
	for(int z=1;z<13;z++)
	{
		...
		outfile << "SuperX3 D" << z << " back side" << endl;
		for(int q=1;q<5;q++)
		{
			for(int t=1;t<5;t++)
			{
				...
				Int_t nPeaks = spec->Search(E_spec[z][q][t],1, "", 0.1);
				...
				new_SX3_cal_params[z][q][t][0] = slope2;
				new_SX3_cal_params[z][q][t][1] = intercept2;
				...
				outfile << q << "      " << t << "      " << slope2 << "      " << intercept2 << endl;
			}
		}
	}
