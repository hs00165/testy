...
*/
	...
	TH2D* front_back = new TH2D("front_back", "front_back", 400, 0, 20, 400, 0, 20);
			...
			for(int a =0;a<dE_strip_DownB.size();a++)
			{
				for(int b =0;b<E_strip_DownB.size();b++)
				{
					for(int c =0;c<E_cal_back_strip_DownB.size();c++)
					{
						...
						if(SX3_prots_down->IsInside(E_en_DownB.at(b),cal_energy_X3.at(a)))
						{

							//if(( (detID_DownB == 1 && E_strip_DownB.at(b) != 3) || (detID_DownB == 5 && E_strip_DownB.at(b) != 1) || (detID_DownB == 6 && E_strip_DownB.at(b) != 4) || (detID_DownB <= 8) ) && E_en_DownB.at(b) <= E_cal_back_en_DownB.at(c) + 0.2 && E_en_DownB.at(b) >= E_cal_back_en_DownB.at(c) - 0.2) front_back->Fill(E_en_DownB.at(b),E_cal_back_en_DownB.at(c));
							...
							{
								if(E_en_DownB.at(b) <= E_cal_back_en_DownB.at(c) + 0.2 && E_en_DownB.at(b) >= E_cal_back_en_DownB.at(c) - 0.2) kinematics1->Fill(angle_DownB,E_cal_back_en_DownB.at(c)+cal_energy_X3.at(a));
								OR_flag = 1;

								q_Value->Fill(angle_DownB, qValue);

								front_back->Fill(E_en_DownB.at(b),E_cal_back_en_DownB.at(c));
