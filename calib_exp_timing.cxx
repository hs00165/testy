...
*/
	...
	TH2D* q_Value2 = new TH2D("q_Value2", "q_Value2", 180, 0, 180, 200, -10, 10);
	...
	TH2D* tIC_dE_dE2 = new TH2D("tIC_dE_dE2", "tIC_dE_dE2", 1000, -23000, -17000, 5000, 0, 50000);
	...
	//timing plots!!!
	TH1D* timing1 = new TH1D("timing1", "timing1", 500, -200, 5000);
	TH1D* timing2 = new TH1D("timing2", "timing2", 500, -200, 5000);
	TH1D* timing3 = new TH1D("timing3", "timing3", 1000, 8000, 20000);
   ...
   SIDAR_prots->SetPoint(12,0.723695,2.18029);
	...
	for(unsigned long long int i=0;i<nEntries;i++)
	{
		...
		//if((trig&2)==2) cout << UpMCP << " " << DownMCP <<" "<< XF_scint <<" "<< RF <<" "<< MTDC_e1up.size() << endl;


		if(UpMCP != 0 && DownMCP != 0 && RF != 0 && XF_scint != 0 && MTDC_e1up.size() >= 1)
		{
			timing3->Fill((RF - UpMCP) - (RF - MTDC_e1up.at(0)));
				...
				for(int b =0;b<E_strip_UpB.size();b++)
				{
					for(int c =0;c<E_cal_back_strip_UpB.size();c++)
					{
						...
						if(((dE_strip_UpB.at(a) == 2*(5-E_strip_UpB.at(b)) || dE_strip_UpB.at(a) == 2*(5-E_strip_UpB.at(b))-1) && cal_energy_BB10.at(a)>=0.4) && SX3_prots->IsInside(E_cal_back_en_UpB.at(c),cal_energy_BB10.at(a)))
						{
							...
							if(var_cut4->IsInside(td1,IC_dE1)) //(var_cut4->IsInside(td1,IC_dE1))        && var_cut3->IsInside(td,IC_dE1)
							{
							...
							if(td2 >= 440 && td2 <= 520)
							{
								kinematics2->Fill(angle_UpB,E_cal_back_en_UpB.at(c)+cal_energy_BB10.at(a));
								q_Value2->Fill(angle_UpB, qValue);
							}
						}
					}
				}
				...
				for(int b =0;b<E_strip_DownB.size();b++)
				{
					...
					//{
						...
						if(SX3_prots_down->IsInside(E_en_DownB.at(b),cal_energy_X3.at(a)) && E_cal_back_strip_DownB.size() >= 1)
						{
							...
							qValue = (1.012)*(E_cal_back_strip_DownB.at(0)+cal_energy_X3.at(a)) - 45.36 - 13.316*sqrt(E_en_DownB.at(b)+cal_energy_X3.at(a))*cos(angle_DownB*(3.1415/180.0));

							if(( (detID_DownB == 1 && E_strip_DownB.at(b) != 3) || (detID_DownB == 5 && E_strip_DownB.at(b) != 1) || (detID_DownB == 6 && E_strip_DownB.at(b) != 4) || (detID_DownB <= 8) ) && var_cut4->IsInside(td1,IC_dE1))  //var_cut4->IsInside(td1,IC_dE1)        && var_cut3->IsInside(td,IC_dE1)
							{
								kinematics1->Fill(angle_DownB,E_cal_back_strip_DownB.at(0)+cal_energy_X3.at(a));
								OR_flag = 1;
							...
							if(td2 >= 440 && td2 <= 520)
							{
								kinematics2->Fill(angle_DownB,E_cal_back_strip_DownB.at(0)+cal_energy_X3.at(a));
								q_Value2->Fill(angle_DownB, qValue);
							}
						}
					//}
				}
			...
			for(int k=0;k<dE_strip_SIDAR.size();k++)
			{
				for(int l=0;l<E_strip_SIDAR.size();l++)
				{
					if(fabs(dE_strip_SIDAR.at(k)-E_strip_SIDAR.at(l))<=1 && cal_dE_energy_SIDAR.at(k) >= 0.4 && cal_E_energy_SIDAR.at(l) >= 0.8 && SIDAR_prots->IsInside(cal_E_energy_SIDAR.at(l),cal_dE_energy_SIDAR.at(k)))
					{
						...
						if(td2 >= 440 && td2 <= 520)
						{
							kinematics2->Fill(angle_SIDAR,cal_dE_energy_SIDAR.at(k)+cal_E_energy_SIDAR.at(l));
							q_Value2->Fill(angle_SIDAR, qValue);
						}
					}
				}
			}
		...
		if(UpMCP != 0 && DownMCP != 0 && RF != 0 && XF_scint != 0 && MTDC_e1up.size() >= 1)
		{
			if(OR_flag == 1) timing1->Fill(UpMCP - RF);
			timing2->Fill(UpMCP - RF);
		}



		//what does dE vs total time of flight look like when gated on 84Se-like beam?
		if(td2 >= 440 && td2 <= 520 && td1 != 0 && OR_flag == 1)
		{
			tIC_dE_dE->Fill(td1,IC_dE1);
		}
		if((td2 <= 440 || td2 >= 520) && td1 != 0 && OR_flag == 1)
		{
			tIC_dE_dE2->Fill(td1,IC_dE1);
		}
		if(td1 != 0) IC_dE_dE->Fill(td1,IC_dE1);
