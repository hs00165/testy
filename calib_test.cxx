...
void calib_test()
{
	...
	ifstream SIDAR_calib_params ("SIDAR_fineEcal.txt");
	...
	ifstream BB10_calib_params ("BB10_fineEcal.txt");
	...
	ifstream SX3_calib_params ("SX3rs_fineEcal.txt");
...
*/
	...
	TH2D* E_vs_strip = new TH2D("E_vs_strip" ,"E_vs_strip", 49, 0, 49, 1000, 0, 10);

	TH2D* E_vs_pos_test = new TH2D("E_vs_pos_test" ,"E_vs_pos_test", 150, -0.5, 1.5, 1000, 0, 10);
   ...
   SX3_prots_down->SetPoint(13,0.859417,10.4571);
	...
	for(unsigned long long int i=0;i<nEntries;i++)
	{
		...
		for( int l=0;l<BarrelUp_vect_size;l++)
		{
			...
			//}
/*
			for(int b=0;b<E_en_UpB.size();b++)
			{
				if(detID_UpB==1) E_vs_strip->Fill(E_strip_UpB.at(b),E_en_UpB.at(b));
				if(detID_UpB==2) E_vs_strip->Fill(E_strip_UpB.at(b)+4,E_en_UpB.at(b));
				if(detID_UpB==3) E_vs_strip->Fill(E_strip_UpB.at(b)+8,E_en_UpB.at(b));
				if(detID_UpB==4) E_vs_strip->Fill(E_strip_UpB.at(b)+12,E_en_UpB.at(b));
				if(detID_UpB==5) E_vs_strip->Fill(E_strip_UpB.at(b)+16,E_en_UpB.at(b));
				if(detID_UpB==6) E_vs_strip->Fill(E_strip_UpB.at(b)+20,E_en_UpB.at(b));
				if(detID_UpB==7) E_vs_strip->Fill(E_strip_UpB.at(b)+24,E_en_UpB.at(b));
				if(detID_UpB==7) E_vs_strip->Fill(E_strip_UpB.at(b)+28,E_en_UpB.at(b));
				if(detID_UpB==8) E_vs_strip->Fill(E_strip_UpB.at(b)+32,E_en_UpB.at(b));
				if(detID_UpB==10) E_vs_strip->Fill(E_strip_UpB.at(b)+36,E_en_UpB.at(b));
				if(detID_UpB==11) E_vs_strip->Fill(E_strip_UpB.at(b)+40,E_en_UpB.at(b));
				if(detID_UpB==12) E_vs_strip->Fill(E_strip_UpB.at(b)+44,E_en_UpB.at(b));
			}
*/
		...
		for( int l=0;l<BarrelDown_vect_size;l++)
		{
			...
			for(int b=0;b<E_en_DownB.size();b++)
			{
			...
			for(int a=0;a<E_en_DownB.size();a++)
			{
				...
				{
					if((detID_DownB==1) && cal_energy_X3.at(b)>=1 && E_en_DownB.at(a)>=1 && SX3_prots_down->IsInside(E_en_DownB.at(a),cal_energy_X3.at(b))) 
					{
						if((trig&2)==2) E_vs_pos_test->Fill(E_pos_DownB.at(b), E_en_DownB.at(b)+cal_energy_X3.at(a));
