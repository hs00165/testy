...
*/
   ...
   SIDAR_prots->SetPoint(12,0.723695,2.18029);
	...
	TFile write("Downstream_position_plots.root", "recreate");

	//TF1 *funcL = new TF1("flatTopGaussLeft", flatTopGaussLeft, 
	TF1 *funcR = new TF1("flatTopGaussRight", flatTopGaussRight, 0.8, 1.2, 10);
	...
	for(int d=1;d<13;d++)
	{
		for(int s=1;s<5;s++)
		{
			...
			cout << "D"<< d << "S" << s << "  " << Pos_spec[d][s]->GetBinCenter(left_shoulder) << "   " << Pos_spec[d][s]->GetBinCenter(right_shoulder) << endl;

			funcR->SetParameters(140,1.05,0.016);

			//Pos_spec[d][s]->Fit("flatTopGaussRight", "r");
